/* =========================================================
   EVERYTHING EVERYTHING — SITE JAVASCRIPT
   Product catalogue, category filter, cart, WhatsApp
   checkout and Paystack checkout.
========================================================= */

/* CONFIGURATION */

const PAYSTACK_PUBLIC_KEY = "pk_test_PASTE_YOUR_PAYSTACK_PUBLIC_KEY_HERE";
const WHATSAPP_NUMBER = "233595485044";
const CART_STORAGE_KEY = "everythingEverythingCart";

/* PRODUCT DATABASE
   category = display label shown on the card
   type     = groceries | clothing | jewelry | gifts (used for filtering)
   Replace image with a real path any time, e.g. "assets/products/rice.jpg" */
...
function addToCart(item) {
    ...
    saveCart();
    updateCart();
    showCartToast(`${cartItem.name} added to cart`);
}
...
function renderProducts(containerSelector, products = PRODUCTS) {
    ...
    if (!available.length) {
        container.innerHTML = `<p style="color:#64748b;padding:20px 0;">No products available.</p>`;
        return;
    }

    container.innerHTML = available.map(product => createProductCard(product)).join("");
...
function initializeProductButtons() {
    const buttons = document.querySelectorAll(".ee-add-fashion-cart");

    buttons.forEach(button => {
        if (button.dataset.cartListener === "true") return;
...
function initializeSizeButtons() {
    const buttons = document.querySelectorAll(".ee-size-options button");

    buttons.forEach(button => {
        if (button.dataset.sizeListener === "true") return;
        ...
        button.dataset.sizeListener = "true";
    });
}
...
function initializeCategoryFilters() {
    ...
    buttons.forEach(button => {
        if (button.dataset.filterListener === "true") return;

        button.addEventListener("click", function () {
            const filter = this.dataset.category || "all";
        ...
        button.dataset.filterListener = "true";
    });
}
...
function checkoutPaystack() {
    ...
    if (!name || !location || !email) {
        alert("Please enter your name, delivery location and email.");
        return;
    }

    if (!PAYSTACK_PUBLIC_KEY || PAYSTACK_PUBLIC_KEY.includes("PASTE_YOUR")) {
        alert("Your Paystack public key has not been added yet.");
        return;
    }

    if (typeof PaystackPop === "undefined") {
        alert("Paystack could not be loaded. Please refresh the page.");
        return;
    }
    ...
    popup.newTransaction({
        ...
        onSuccess: function (transaction) {
            handleSuccessfulPayment(transaction.reference || reference);
        },
        onCancel: function () {
        ...
        onError: function (error) {
            ...
            alert("There was a problem opening Paystack.");
        }
    });
}

function createPaymentReference() {
    return "EE_" + Date.now() + "_" + Math.floor(Math.random() * 100000);
}

function handleSuccessfulPayment(reference) {
    const order = {
        reference,
        customer: {
            name: getInputValue("customerName"),
            location: getInputValue("customerLocation"),
    ...
    // Saved locally for now — can be connected to a backend / Paystack verification later.
    localStorage.setItem("lastEverythingEverythingOrder", JSON.stringify(order));
